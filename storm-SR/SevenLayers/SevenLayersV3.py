...
class SevenLayersV3(nn.Module):
    def __init__(
        ...
        super(SevenLayersV3, self).__init__()
    ...
    def generate_Encoder(self, f_maps, num_layers=0):
        ...
        for idx in range(1, len(f_maps)):
            encoder_layer = EncoderLayer(
                ...
                if_downsample=False if idx==1 else True
            )
    ...
    def generate_Decoder(self, f_maps):
        ...
        for idx in range(1, len(f_maps)):
            decoder_layer = DecoderLayer(
                in_channels=f_maps[idx-1],
                out_channels=f_maps[idx],
                if_up_sample=False if idx==len(f_maps)-1 else True
            )
...
class EncoderLayer(nn.Module):
    def __init__(
        ...
        self.down_sample = nn.MaxPool3d(kernel_size=(2,1,1))
        # self.down_sample = nn.Conv3d(out_channels, out_channels, kernel_size=(3, 3, 3), stride=(2, 1, 1), padding=(1, 1, 1))
