...
from SevenLayers.transformer.utils import get_root_logger
...
class DecoderBasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.merge_mlp = nn.Linear(2 * dim, dim, bias=False)
    ...
    def forward(self, x, encoder_features):
        ...
        if not self.if_first:
            encoder_features = rearrange(encoder_features, 'b c d h w -> b d h w c')
        ...
        for blk in self.blocks:
            x = blk(x, attn_mask)
        x = x.view(B, D, H*2, W*2, -1)
        x = rearrange(x, 'b d h w c -> b c d h w')
        return x


class MiddelBasicLayer(nn.Module):
    """ A basic Swin Transformer layer for one stage.
